...
DATE_CALC AS(
  ...
  BizDate, FORMAT_DATE('%b %Y', PCD.BizDate) as Month,
	...
	-- splitting WeekYear
	-- CONCAT(LPAD(CAST(SPLIT(WeekYear) AS STRING)))
	CONCAT('Wk', 
		LPAD(
			CAST(
				SPLIT(
...
group by PCD.BizDate, WeekYear, Wk, `Calendar Day Week`, BM.Banner, Location, BM.Branch_Name, PCD.dept, H_IM.deptdesc, PCD.SubDept, H_IM.subdeptdesc, PCD.Category, H_IM.categorydesc, H_IM.ItemLink, PCD.Itemcode, PCD.description, VDR.Code, VDR.Name, PCD.consign, H_IM.costmarginvalue, PSC.LastCost, PSC.FIFOCost, BGT.sales_budget
...
-- where (H_IM.Dept LIKE ('1%') OR H_IM.Dept LIKE ('2%') OR H_IM.Dept LIKE ('3%') OR H_IM.Dept LIKE ('4%') OR H_IM.Dept LIKE ('5%') OR H_IM.Dept LIKE ('6%'))
group by SRP_PS.Date, WeekYear, Wk, `Calendar Day Week`, BM.Banner, SRP_PS.Branch_Code, BM.Branch_Name, H_IM.dept, H_IM.deptdesc, H_IM.SubDept, H_IM.subdeptdesc, H_IM.Category, H_IM.categorydesc, ItemLink, PLU_Number, Descriptor, VDR.Code, VDR.Name, H_IM.consign, BGT.sales_budget
...
group by AGG_GR.YearMonthDay, WeekYear, Wk, `Calendar Day Week`, BM.Banner, AGG_GR.Site, BM.Branch_Name, AGG_GR.Article, H_IM.dept, H_IM.deptdesc, H_IM.SubDept, H_IM.subdeptdesc, H_IM.Category, H_IM.categorydesc, H_IM.Description, H_IM.ItemLink, VDR.Code, VDR.Name, BGT.sales_budget
